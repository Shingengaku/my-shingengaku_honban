...
interface Application {
    ...
    payment_key?: string; // Generated by backend or derived
...
{{payment_link_section}}`
        ...
        try {
            ...
            if (settingsRes.ok) {
                ...
                if (Array.isArray(val)) {
                    ...
                    // Old Format (Object -> Array conversion)
                    const linksObj = val || {};
    ...
    const generateKeyCandidates = () => {
        // Use the explicitly defined Product Name Master list
    ...
    const handleResend = async (id: string) => {
        if (!confirm('メールを再送しますか？（内容は「閲覧」ボタンで確認できます）')) return;
        try {
    ...
    const ignoreDuplicate = async () => {
        ...
        setLoading(true);
        try {
            // Append 'ignore_duplicate' to tags
    ...
    // Helper to extract fields from Key string like "【Rank】Venue/Social"
    ...
    const handleKeyChange = (key: string) => {
        // 1. Try to find in Product Master (PaymentLinks)
        ...
        // 2. Legacy Parse (if key contains format info, it might override or be fallback)
    ...
    const exportCSV = (useFilter: boolean = true) => {
        const targetApps = useFilter ? [...filteredApps] : [...apps]; // Use copy for sorting
            ...
            {showSettingsModal && (
                <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full flex items-center justify-center z-50">
                    <div className="bg-white p-5 rounded-lg shadow-xl w-[800px] max-h-[90vh] overflow-y-auto">
                        ...
                        <div className="mb-6">
                            <h4 className="font-bold text-gray-700 mb-2">自動返信メールテンプレート</h4>
                            <div className="bg-yellow-50 p-3 rounded text-xs mb-2">
                                利用可能な変数: {'{{name}}'}, {'{{rank}}'}, {'{{venue}}'}, {'{{social_venue}}'}, {'{{amount}}'}, {'{{payment_link_section}}'}
                            </div>
                            <div className="mb-2">
                                <label className="block text-sm text-gray-600 text-xs">件名</label>
                                <input
                                    className="border w-full p-2 rounded"
                            ...
                            <div>
                                ...
                                <textarea
                                    className="border w-full p-2 rounded h-40 font-mono text-sm"
