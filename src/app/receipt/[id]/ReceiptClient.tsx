...
export default function ReceiptClient({ data }: { data: ReceiptData }) {
    const [docType, setDocType] = useState<DocType>(data.initialDocType || 'receipt');
    const [addressee, setAddressee] = useState(data.input_name);
    const [honorific, setHonorific] = useState('御中');
    const [description, setDescription] = useState(initialDescription);

    // タグから初期値を抽出
    const initialIssueDate = data.tags.find(t => t.startsWith('rd:'))?.split(':')[1] || new Date().toISOString().slice(0, 10);
    const initialPaymentMethod = data.tags.find(t => t.startsWith('pm:'))?.split(':')[1] || '銀行振込';

    const [issueDate, setIssueDate] = useState(initialIssueDate);
    const [dueDate, setDueDate] = useState(''); // 請求書用: お支払い期限
                        ...
                        <div className="space-y-4">
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">
                                    {docType === 'receipt' ? '領収日' : '発行日'} {!data.isAdmin && <span className="text-[10px] text-gray-400 font-normal ml-1">(事務局設定済み)</span>}
                                </label>
                            ...
                            {docType === 'invoice' && (
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-1">お支払い期限</label>
                                    <input 
                                        type="date" 
                                        value={dueDate} 
                                        onChange={e => setDueDate(e.target.value)} 
                                        className="w-full bg-white border-gray-300 rounded-md shadow-sm text-sm p-2 border" 
                                    />
                                </div>
                            )}
                            <div>
            ...
            */}
            ...
            >
                ...
                <div style={{ position: 'absolute', top: '55mm', right: '35mm', display: 'flex', alignItems: 'flex-end', gap: '8px' }}>
                    <span style={{ fontSize: '14px', letterSpacing: '0.1em' }}>{docType === 'receipt' ? '発行日' : '請求日'}</span>
                    <span style={{ 
                        fontSize: '15px', 
                ...
                <div style={{ position: 'absolute', top: '90mm', left: '62mm', width: '185mm' }}>
                    ...
                    {docType === 'invoice' && dueDate && (
                        <>
                            <div style={{ display: 'flex', alignItems: 'center', height: '10mm', paddingLeft: '5mm' }}>
                                <div style={{ fontSize: '12px', width: '90px' }}>お支払い期限</div>
                                <div style={{ fontSize: '14px', fontWeight: 'bold', paddingLeft: '10px' }}>
                                    {dueDate.replace(/-/g, '/')}
                                </div>
                            </div>
                            <div style={{ borderTop: '1px solid #999', width: '100%' }}></div>
                        </>
                    )}
                    ...
                    <div style={{ paddingTop: '8px', fontSize: '13px', letterSpacing: '0.1em' }}>
                        {docType === 'receipt' ? '上記正に領収いたしました。' : '下記の通りご請求申し上げます。'}
                    </div>
                ...
                <div style={{ position: 'absolute', bottom: '30mm', right: '35mm', display: 'flex', alignItems: 'flex-end', gap: '8px' }}>
                    ...
                    />
                        ...
                        {/* お支払い方法・振込先（請求書の場合、振込先を表示） */}
                        {docType === 'invoice' && paymentMethod === '銀行振込' ? (
                            <div style={{ position: 'absolute', bottom: '15mm', left: '110mm', fontSize: '10px', color: '#222', border: '1px solid #ccc', padding: '10px', backgroundColor: '#fdfdfd' }}>
                                <div style={{ fontWeight: 'bold', borderBottom: '1px solid #ccc', marginBottom: '4px', paddingBottom: '2px' }}>【お振込先】</div>
                                <div>福岡銀行（フクオカギンコウ）</div>
                                <div>赤坂門支店（アカサカモンシテン）</div>
                                <div>普通 １８４４７２６</div>
                                <div>カ）フィールドオブドリームス</div>
                            </div>
                        ) : (
                            paymentMethod !== '銀行振込' && (
