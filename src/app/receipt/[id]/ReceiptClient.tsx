...
export default function ReceiptClient({ data }: { data: ReceiptData }) {
    ...
    const initialDescription = data.lecture_fee > 0 && data.social_fee > 0 
        ? '受講費用、懇親会費用として' 
        : (data.lecture_fee > 0 ? '受講費用として' : '懇親会費用として');
    ...
    const isDocIssued = (checkSplitType: SplitType) => {
        const prefix = 'receipted';
        let exactTag = prefix;
    ...
    const isCurrentDocIssued = isDocIssued(splitType);
