...
export async function POST(request: Request) {
    try {
        ...
        // 1. 申込データと設定、ランクを取得
        const [appRes, settingsRes, ranksRes, termsRes, childrenRes] = await Promise.all([
        ...
        children.forEach(child => {
            let childRankId: string | null = null;
            if (child.members?.ranks?.id) {
                childRankId = String(child.members.ranks.id);
            } else if (child.applied_rank_name) {
                const found = ranks.find(r => r.name === child.applied_rank_name);
                if (found) childRankId = String(found.id);
            }
            ...
            if (childMatched) {
                combinedAmount += (Number(childMatched.lecture_fee) || 0) + (Number(childMatched.social_fee) || 0);
            }
        });
